<template>
  <div class="flex w-full bg-gray-200 border-t-4 border-b-4 border-black">
    <!-- Active Tab Display -->
    <div class="flex-grow flex items-center">
      <div class="flex items-center px-4 py-2.5">
        ...
        <div class="flex items-center">
          ...
          <span v-if="state.selected.tab" class="text-sm leading-tight ml-2" :class="fontColor(state.selected.tab.total)">{{ tabTotal }}</span>
        </div>
      </div>
    </div>

    <!-- All tabs dropdown button -->
    <div class="w-12 flex-shrink-0 border-l-4 border-black">
      <button 
        @click="router.push({ name: 'all-tabs' })" 
        class="h-full w-full bg-gray-900 flex items-center justify-center text-white hover:text-gray-100"
      >
        <ChevronDown size="16" />
      </button>
    </div>
