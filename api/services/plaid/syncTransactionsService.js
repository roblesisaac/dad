...
class TransactionSyncService {
  ...
  async _getAndLockItem(item, user) {
    ...
    if (itemData.status === 'in_progress' && await syncSessionService.isSyncRecent(itemData, user)) {
  ...
  async _updateItemAfterSync(item, syncResult, sync_id, syncTime) {
    ...
    // Always update syncData.lastSyncTime so frontend can display it
    updateData.syncData = {
      ...(item.syncData || {}),
      lastSyncTime: syncTime || Date.now()
    };
