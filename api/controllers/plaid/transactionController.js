...
export default {
  ...
  async getSyncSessionsForItem(req, res) {
    try {
      const user = req.user;
      const { itemId } = req.params;
      const { limit = 20, start } = req.query;
      ...
      const { sessions: syncSessions, lastKey } = await syncSessionService.getSyncSessionsForItem(
        itemId,
        user._id,
        { limit: parseInt(limit), start: start || undefined }
      );
